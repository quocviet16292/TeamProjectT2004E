...
app.get("/Promotion/ServicName/search",function (req,res) {
    ...
    db.query("SELECT TOP 300 a.Name,a.District,a.Avartar,a.Cover,a.Phone,b.TName,c.*,a.Id_District\n" +
        ...
        "WHERE SName LIKE "+key_search,function (err,rows) {
...
app.get("/Promotion/Servis/search",function (req,res) {
    ...
    db.query("SELECT TOP 300 a.Name,a.District,a.Avartar,a.Cover,a.Phone,b.TName,c.*,a.Id_District\n" +
        ...
        "              INNER JOIN T2004E_Nhom1_District d\n" +
        "               ON d.Id =a.Id_District \n" +
        "               WHERE NOT EXISTS (SELECT * FROM T2004E_Nhom1_MU_Services cc WHERE cc.Id = c.Id AND cc.SPrice > c.SPrice) AND b.TName LIKE "+key_search,function (err,rows) {
        if(err) res.send("Ko co ket qua");
        else
            res.render("artistbeauty",{
                artist: rows.recordset
...
app.get("/Promotion/District-2/search",function (req,res) {
    let key_search = "N'%"+req.query.keyword+"%'";
    db.query("SELECT top 300 a.Id_District,a.Name,a.District,a.Avartar,a.Cover,a.Phone,b.TName,c.*\n" +
        "\t\tFROM T2004E_Nhom1_Artist a\n" +
        "\t\tINNER JOIN T2004E_Nhom1_MU_Services c\n" +
        "\t\tON c.Id = a.Id\n" +
        "\t\tINNER JOIN T2004E_Nhom1_MU_Type b\n" +
        ...
        "\t\tINNER JOIN T2004E_Nhom1_District d\n" +
        "\t\tON d.Id =a.Id_District\n" +
        "\t\tWHERE NOT EXISTS (SELECT * FROM T2004E_Nhom1_MU_Services cc WHERE cc.Id = c.Id AND cc.SPrice > c.SPrice) AND District LIKE "+key_search,function (err,rows) {
        if(err) res.send("Ko co ket qua");
        else
            res.render("Promotion-son",{
                sps: rows.recordset
...
app.get('/booking/:serviceID', async (req,res)=>{
    ...
    let sql_text7 = "SELECT * FROM SELECT * FROM T2004E_Nhom1_District WHERE IdCity =" +cityID;
    let sv = "";
    let art = "";
    let allsv ="";
    let city ="";
    ...
    await db.query(sql_text7).then(rs=>{
        district2 = rs;
    }).catch(function (err){
        console.log(err);
    })
